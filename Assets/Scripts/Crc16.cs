...
public class Crc16 : MonoBehaviour
{
    ...
    ///一个crc16类包装各种crc16算法
    ///CRC16实用程序，使用初始CRC值为0的反向x^16+x^15+x^2+1（0xA001）多项式
    internal static class Standard
    {
        // 使用反转x^16+x^15+x^2+1多边形/0xA001的预计算表
        ...
        /// 计算所提供字节的CRC16
        /// </summary>
        /// <param name="bytes">字节到crc</param>
        /// <returns>所提供字节的CRC16</returns>
        ...
        /// 计算所提供字节的CRC16
        /// </summary>
        /// <param name="bytes">用于计算CRC的缓冲区</param>
        /// <param name="start">计算CRC的起始索引</param>
        /// <param name="length">要计算CRC的缓冲区长度</param>
        /// <returns>指定的CRC</returns>
    ...
    /// <summary>
    /// CRC16分布式网络协议（DNP）实用程序，使用初始CRC值为0的反x^16+x^13+x^12+x^11+x^10+x^8+x^6+x^5+x^2+1（0xA6BC）多项式
    /// </summary>
    internal static class Dnp
    {
        // 使用反转x^16+x^13+x^12+x^11+x^10+x^8+x^6+x^5+x^2+1多边形/0xA6BC的预计算表
        ...
        /// <summary>
        ///使用初始CRC值为0的反向x^16+x^13+x^12+x^11+x^10+x^8+x^6+x^5+x^2+1（0xA6BC）多项式计算CRC16分布式网络协议（DNP）实用程序
        /// </summary>
        /// <param name="bytes">字节到crc</param>
        /// <returns>所提供字节的CRC16</returns>
        internal static ushort ComputeChecksum(params byte[] bytes)
        ...
        /// <param name="bytes">用于计算CRC的缓冲区</param>
        /// <param name="start">计算CRC的起始索引</param>
        /// <param name="length">用于计算CRC的缓冲区的长度</param>
        /// <returns>指定的CRC</returns>
        internal static ushort ComputeChecksum(byte[] bytes, int start, int length)
        {
            if (bytes == null) { throw new ArgumentNullException(nameof(bytes)); }
    ...
    /// <summary>
    ///CRC16 CCITT实用程序，使用初始CRC值为0的反向x^16+x^15+x^2+1多项式（0x8408）
    /// </summary>
    internal static class CcittKermit
    {
        // 使用反转x^16+x^15+x^2+1多边形/0x8408的预计算表
        ...
        /// <summary>
        /// 使用初始值0计算所提供字节的CRC16 CCITT Kermit
        /// </summary>
        /// <param name="bytes">字节到crc</param>
        /// <returns>所提供字节的CRC16</returns>
        internal static ushort ComputeChecksum(params byte[] bytes)
    ...
    /// <summary>
    /// 使用x^16+x^15+x^2+1多项式的CRC16 CCITT实用程序，初始CRC值为0
    /// </summary>
    internal static class CcittInitialZero
    {
        internal static ushort InitialValue = ushort.MinValue;
        /// <summary>
        /// 使用初始值0计算所提供字节的CRC16 CCITT
        /// </summary>
        /// <param name="bytes">字节到crc</param>
        /// <returns>所提供字节的CRC16 CCITT</returns>
    ...
    {
        // 使用x^16+x^15+x^2+1多项式/0x1021的预计算表
    ...
    /// <summary>
    /// 使用给定的算法计算指定字节的CRC 16校验和
    /// </summary>
    /// <param name="algorithm">使用CRC16算法</param>
    /// <param name="bytes">用于计算CRC的缓冲区</param>
    /// <returns>指定的CRC</returns>
